...
### Fixed
- **`healthScoreStream` (and other event streams) on Android no longer silently drop events emitted from background threads (Bug-025).** Flutter's `EventChannel.EventSink` methods are `@UiThread`. polyfence-core since 1.0.11 runs `emitHealthScore` on a dedicated background Thread (Bug-010 fix), so the Flutter bridge's `onPerformanceEvent` handler was calling `EventSink.success(...)` from that background thread — throwing `IllegalStateException` that core caught + logged, leaving `healthScoreStream` silent for the whole session. All four bridge delegate callbacks (`onLocationUpdate`, `onGeofenceEvent`, `onPerformanceEvent`, `onError`) now marshal deliveries through a hoisted `Handler(Looper.getMainLooper())`, matching the pattern already used for the `debugInfo` result callback. iOS is unaffected — `FlutterEventSink` on iOS has no equivalent thread constraint.
