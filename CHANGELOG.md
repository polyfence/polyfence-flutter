...
### Documented
- **`onZoneEnter` / `onZoneExit` convenience streams are now in the README (Bug-027).** Both streams have existed in `PolyfenceService` and work — they filter `onGeofenceEvent` strictly for `GeofenceEventType.enter` / `.exit` respectively — but were undocumented on Flutter. RN's README lists them in its Events table; Flutter now mentions them right after the "Listen for Events" switch example so developers reading the docs see the shortcut instead of writing the manual filter themselves. **Cross-bridge behavioural note:** the RN variants also include `recoveryEnter` / `recoveryExit`; Flutter's are strict enter/exit only. Recovery / dwell / signal-lost events still require `onGeofenceEvent`. Documented in the README, no code change.
- **`addZone` duplicate-ID behaviour is now on the public docstring (Bug-024).** Calling `addZone` with a `Zone.id` already being monitored silently overwrites the previous zone entry — no error is thrown. **Re-adding also resets the persisted INSIDE/OUTSIDE state for that zone (and on iOS, its confidence state).** If the device is currently inside the zone, the next reconciliation may fire a fresh `enter` / `recoveryEnter` event. In-place metadata edits without a re-enter are a known limitation. Behaviour is unchanged; only the documentation is new.

### Changed
- **polyfence-core bumped 1.0.11 → 1.0.14.** Picks up (a) `applyRemoveZoneDirect` / `applyClearZonesDirect` companion helpers used by the Bug-021 fix below, (b) the degraded-GPS staleness watchdog + `signalLost` / `signalRestored` events (1.0.13 — the event-forwarding wiring landed on the bridge in a prior release but stayed dormant because core 1.0.11 never emitted those events; the bump activates that surface), and (c) the iOS `PolyfenceDebugCollector.getErrorHistory` filter fix (1.0.14) that the Bug-026 iOS parity below depends on. See polyfence-core 1.0.13 + 1.0.14 CHANGELOGs.
...
### Fixed
- **`errorHistory()` now returns real data on both platforms (Bug-026).** Three parallel defects (Android bridge, iOS bridge stub, iOS core filter), all fixed together.
