...
- **Bridge attach/detach lifecycle (`setBridgeAttached`).** The plugin now signals to core when its Flutter delivery sink is or is not attached: attached on `initialize()`, detached on `Polyfence.dispose()`, on `onDetachedFromEngine` (Android hot-reload / engine teardown), and on `applicationWillTerminate` (iOS). While attached, geofence events reach the consumer live. While detached (with `pendingEventsQueueSize > 0`), events are persisted to the durable queue instead of dropping silently at the delegate boundary. On Android, a torn-down `EventSink` also auto-flips the signal via core's delegate exception-catch — belt-and-braces so a crashed Flutter engine still triggers the persist branch.
