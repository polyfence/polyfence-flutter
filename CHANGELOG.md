...
### Changed
- **polyfence-core bumped 1.0.11 → 1.0.13.** Picks up the new `applyRemoveZoneDirect` / `applyClearZonesDirect` companion helpers (see next bullet), plus the degraded-GPS staleness watchdog + `signalLost` / `signalRestored` events documented in polyfence-core 1.0.13's CHANGELOG. The event-forwarding wiring for `signalLost` / `signalRestored` landed on the bridge in a prior release but stayed dormant because core 1.0.11 never emitted those events; upgrading to 1.0.13 activates that surface.
